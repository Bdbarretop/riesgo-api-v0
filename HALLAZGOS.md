...
| H7 | Dos instancias distintas de `EvaluadorRiesgo` comparten la misma lista `historial`: `EvaluadorRiesgo("A").anotar(0.5)` deja rastro visible en `EvaluadorRiesgo("B").historial` | `dominio.py` declara `historial = []` como atributo **de clase** (línea 14), por lo que la lista existe una sola vez y todas las instancias la mutan | M3 · 3. Componentes: atributos de clase | `v0-semilla` | `python -c "from dominio import EvaluadorRiesgo; a=EvaluadorRiesgo('A'); b=EvaluadorRiesgo('B'); a.anotar(0.5); print(len(b.historial))"` | `1` | Se saca el historial de la clase: cada `EvaluadorRiesgo` guarda su propio historial en `__init__`, y las anotaciones que sobreviven entre peticiones viven en un `RepositorioEvaluaciones` inyectado como colaborador opcional |
